...
def _on_change(gpio, level, tick):
    ...
    # 10 microseconds per data bit, so number of bits since last state change is time difference/10
    num_bits = round((time_elapsed)/10)
    # enter the level *before* this state change which is the inverse of current change.
...
if __name__ == "__main__":
    # Sbus pin GPIO-4
    sbus_reader = SbusReader(gpio_pin=4)
    sbus_reader.begin_listen()

    try:
        time.sleep(0.1)
        sbus_reader.display_latest_packet_curses()
    except KeyboardInterrupt:
        print("Программа остановлена пользователем")
    finally:
        sbus_reader.end_listen()
